        ...
        for file in filter(re_tar_gz.search, files):
            try: 
                tarfile.open(
                    ...
                    mode='r:gz'
                    ).extractall(path=directory, filter='data')
            except: 
                print("Is it a tar.gz file?")
            else: 
...
unpack_archives_in_unpacked_debs()
    ...
    # https://github.com/avinashkranjan/Amazing-Python-Scripts/blob/main/Audio%20to%20Text%20Convertor/code.py#L28 -- пример скрипта
...
# os.listdir -- возвращает list
# os.scandir -- возвращает iterator of DirEntry   
